...
"""
        ...
        prompt = f"""
You are an expert HR evaluator. Your task is to analyze a specific job section and a candidate's documents to determine if the candidate qualifies for the position.
...
{{
    ...
    "graduation_date_found": "Date found in documents or 'Not specied",
    "post_qualifying_experience_years": "Number of years calculated or 'Unable to calculate'",
...
def main():
    ...
    # Page configuration
    st.set_page_config(
        page_title="HR Hiring Automation PoC",
        page_icon="👥",
        layout="wide"
    )
    
    # Header
    st.title("AI HR Recruitment Solution")
    st.markdown("AI HR Recruitment Solution")
    ...
    with st.sidebar:
        ...
        3. AI will identify all job positions
        4. Select the specific job position
        5. Upload candidate's resume (PDF)
        6. Upload qualification certificate (PDF)
        7. Click 'Evaluate Candidate'
        8. Review the AI evaluation results
        """)
    ...
    if api_key:
        ...
        st.header("Step 1: Upload Job Advertisement")
        ...
        if job_file and not st.session_state.jobs_extracted:
            with st.spinner("Extracting job positions from advertisement using AI..."):
                try:
                    ...
                    # Extract available jobs using GPT-4
                    st.info("🤖 Analyzing document with AI to identify job positions...")
                    ...
                    st.success(f"✅ AI identified {len(available_jobs)} job position(s)")
        ...
        if st.session_state.jobs_extracted and st.session_state.available_jobs:
            st.header("🎯 Step 2: Select Job Position")
            ...
            selected_job_title = st.selectbox(
                "Choose the position you want to hire for:",
                options=job_options,
                index=0,
            ...
            st.header("👤 Step 3: Upload Candidate Documents")
            ...
            with col2:
                st.subheader("🎓 Qualification Certificate")
            ...
            if resume_file:  # Certificate is optional
                st.header("🚀 Step 4: Candidate Evaluation")
                
                # col1, col2 = st.columns([3, 1])
                # with col1:
                #     st.info(f"**Selected Position:** {selected_job_title}")
                    ...
                    if st.button("Evaluate Candidate", type="primary", use_container_width=True):
                        ...
                        with st.spinner("Processing documents and evaluating candidate..."):
                            try:
                                ...
                                st.info("🤖 Getting GPT-4 evaluation...")
                                ...
                                with col1:
                                    if evaluation_result["qualifies"].lower() == "yes":
                                        ...
                                        st.balloons()  # Celebration for qualified candidates
                                    else:
                                        st.error("❌ **NOT QUALIFIED**")
                                
                                with col2:
                                    st.metric("Position", selected_job_title)
                                ...
                                # JSON Response for transparency
                ...
                st.info("📤 Please upload the candidate's resume to proceed with evaluation")
    ...
    if st.session_state.jobs_extracted:
        st.markdown("---")
        if st.button("🔄 Start Over (Upload New Job Advertisement)", type="secondary"):
            st.session_state.jobs_extracted = False
            st.session_state.available_jobs = []
            st.session_state.selected_job = None
            st.rerun()
    ...
    # Footer
    st.markdown("---")
    st.markdown("*Built with Streamlit, GPT-4, and pdfplumber | Enhanced Multi-Position Support*")
